...
# calculate the derivative of the mean function at TESTING data points
def NIGP_f_grad_test(X_test, X_train, Y_train, l=1.0, sigma_f=1.0, sigma_y=1e-8):
    """
        NOT used
    """
    # f_mean = k(xts, xtr) * inv[K + Sigma_y*I_N] * y  -- mean func of f at TESTING points

    VAR_X_train = tf.placeholder(tf.float64, shape=X_train.shape)
    VAR_X_test = tf.placeholder(tf.float64, shape=X_test.shape)

    K_Xs_Xtr = kernel_Tensor(X1=VAR_X_test, X2=VAR_X_train, l=l, sigma_f=sigma_f)
    K_Xtr_Xtr = kernel_Tensor(X1=VAR_X_train, l=l, sigma_f=sigma_f)

    K_reg = K_Xtr_Xtr + sigma_y ** 2 * tf.eye(len(X_train[1]), dtype=tf.float64)
    K_inv = tf.linalg.inv(K_reg)

    mean_f = tf.matmul(tf.matmul(K_Xs_Xtr, K_inv), Y_train)

    f_grad = tf.gradients(mean_f, VAR_X_test)
    with tf.Session() as sess:
        f_grad_test = sess.run(f_grad,  feed_dict={VAR_X_test: X_test, VAR_X_train: X_train})
    return f_grad_test[0]
...
def NIGP_posterior(X_s, X_train, Y_train, l=1.0, sigma_f=1.0, sigma_y=1e-8, sigma_x=1e-8):
    ...
    # f_grad_test = NIGP_f_grad_test(X_s, X_train, Y_train, l, sigma_f, sigma_y)
    # SIGMA_x = sigma_x ** 2
    # SIGMA_y = sigma_y ** 2 * np.eye(len(X_s))
    # K_grad_test = f_grad_test.dot(SIGMA_x).dot(f_grad_test.T)
...
def f_nigp_posterior(X_train, Y_train, l=1.0, sigma_f=1.0, sigma_y=1e-8, sigma_x=1.0):
    ...
    # SIGMA_x = sigma_x ** 2
    # reg_grad = f_grad_mean.dot(SIGMA_x).dot(f_grad_mean.T)
    # K_reg = K + sigma_y ** 2 * np.eye(len(X_train)) + reg_grad
    # K_inv = inv(K_reg)
...
if __name__ == "__main__":
    ...
    # mu_prior = np.zeros(X_test.shape)
    ...
    # Noisy training data
    # X_train = np.arange(-4, 3, 0.5).reshape(-1, 1)
    ...
    # mu_s_nigp, cov_s_nigp = NIGP_posterior(X_test, X_train, Y_train, l=l_opt, sigma_f=sigma_f_opt,
    ...
    # plot_gp(mu_post_GPR_fit, cov_post_GPR_fit, X_test, X_train=X_train, Y_train=Y_train, titles='GPR post fit paras')
    #
    # f_mu_gp, f_cov_gp = f_posterior(X_train, Y_train, l=l_opt, sigma_f=sigma_f_opt, sigma_y=noise_y)
    # plot_gp_f(f_mu_gp, f_cov_gp, X_train=X_train, Y_train=Y_train, titles='GPR f dis init_paras')
    #
    # f_mu_nigp, f_cov_nigp = f_nigp_posterior(X_train, Y_train, l=l_opt, sigma_f=sigma_f_opt, sigma_y=noise_y, sigma_x=noise_x)
    # plot_gp_f(f_mu_nigp, f_cov_nigp, X_train=X_train, Y_train=Y_train, titles='NIGPR f dis init_paras')
